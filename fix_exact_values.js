...
let dbContent = fs.readFileSync('d:/ai/ASST/db/settings_db.js', 'utf8');
let modifications = 0;
...
lines.forEach(line => {
    ...
    if (!key) return;
    const val = valParts.join('=').trim(); // Keep everything after the first = exactly as is
    ...
    if (valLower === 'true' || valLower === 'false') {
        type = 'bool';
        formattedVal = valLower === 'true' ? 'true' : 'false';
    } else if (!isNaN(Number(val)) && val.trim() !== '') {
        // Safe to treat as a number
        if (val.includes('.') || Number(val) !== parseInt(val, 10)) {
            type = 'float';
            formattedVal = val;
        } else {
            type = 'int';
    ...
    // Simple regex to locate the block for this key
    const blockRegex = new RegExp(`({\\s*id:[^}]+key:\\s*"\\b${key}\\b"[^}]+type:\\s*)"([^"]+)"([^}]+default:\\s*)([^,]+)(,[^}]+})`, 'm');
    const match = dbContent.match(blockRegex);

    if (match) {
        const currentTypeStr = match[2].trim();
        const currentValStr = match[4].trim();
        let newBlock = match[0];
        let replaced = false;

        if (currentTypeStr !== type) {
            newBlock = newBlock.replace(new RegExp(`type:\\s*"${currentTypeStr}"`), `type: "${type}"`);
            replaced = true;
        }

        // Replace exact default string if it doesn't match
        if (currentValStr !== formattedVal) {
            const safeValStr = currentValStr.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, '\\$&');
            newBlock = newBlock.replace(new RegExp(`default:\\s*${safeValStr}(,|\\n)`), `default: ${formattedVal}$1`);
            replaced = true;
        }

        if (replaced) {
            dbContent = dbContent.replace(match[0], newBlock);
            modifications++;
            console.log(`Updated ${key}: ${currentValStr} -> ${formattedVal} (${type})`);
        }
    } else {
        const looseRegex = new RegExp(`key:\\s*"${key}"`, 'i');
        if (!dbContent.match(looseRegex)) {
            console.log("Could not find key in DB:", key)
        }
    }
});

if (modifications > 0) {
    fs.writeFileSync('d:/ai/ASST/db/settings_db.js', dbContent);
    console.log(`Saved ${modifications} modifications to settings_db.js`);
} else {
